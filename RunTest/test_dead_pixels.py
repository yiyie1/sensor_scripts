...
def SetupGradientConfig(testLib, product, hwid, level):
    ...
    status = testLib.init_gradient_checkerboard_test_based_on_testlevel(c.byref(cb_gradient_config), product, hwid, level)
...
def DeadPixelsGradient(testLib, fullPath, select_test, cb_config):
...
def DeadPixels(testLib, fullPath, select_test, cb_config):
    print("\n--------------------Running Dead Pixels {} Test--------------------------\n".format(select_test));
...
if __name__ == "__main__":
    ...
    if args.test_level == "MTS":
        test_level = TestLevelT.FPC_TEST_LEVEL_MTS
    elif args.test_level == "ITS":
    ...
    testLib = TestLib("prodtestlib.dll")
    testLib.setup_interface()
    ...
    if args.product == "1035" or args.product == "1020" \
        ...
        DeadPixels(testLib, args.image, int(select_test), cb_config)
    else:
        cb_config = SetupGradientConfig(testLib, product_map[args.product], hwid, int(test_level))
        DeadPixelsGradient(testLib, args.image, int(select_test), cb_config)